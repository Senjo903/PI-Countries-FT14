    ...
    <>
      ...
      <div className="body-register">
        <div className="register">
          <h2>
            Register Tourist Activity
          </h2>
          <form onSubmit={handleSubmit} className="register-form">
            <div id="register-input3" className="aling-form">
              <label>
                ...
                (<label className="rejected tipe-text1">
                  the activity already exists
                </label>)}
              </label>
              <div>
                <input id="register-input2" placeholder="name of the tourist activity" className="register-input width-input" onChange={handleChangeActivityName} value={values.activityName} name="activityName" type="text"/>
                </div>
              </div>
            ...
            <div id="register-input4" className="aling-form">
              <label>
                Duration: {values.duration !== ''?(<></>):(<label className="rejected tipe-text1">it is mandatory to specify a duration</label>)}
              </label>
              <div>
                <input id="register-input5" className="register-input width-input" onChange={handleChange} value={values.duration} name="duration" type="text"/>
              </div>
            </div>
            <div id="stations2" className="aling-form">
              <label>
                Station: {values.station !== ''?(<></>):(<label className="rejected tipe-text1">it is mandatory to select a station</label>)}
              </label>
              <div id="stations" className="stations">
                <label id="summer1" onClick={changeStation('summer')} className={values.station === 'summer' ? 'buttonStateUp': 'buttonState'}>
                  Summer <FiSun className='station summer'/>
                </label>
                <label id="fall1" onClick={changeStation('fall')} className={values.station === 'fall' ? 'buttonStateUp': 'buttonState'}>
                  Fall <GiFallingLeaf className='station fall'/>
                </label>
                <label id="winter1" onClick={changeStation('winter')} className={values.station === 'winter' ? 'buttonStateUp': 'buttonState'}>
                  Winter <GiWinterGloves className='station winter'/>
                </label>
                <label id="spring1" onClick={changeStation('spring')} className={values.station === 'spring' ? 'buttonStateUp': 'buttonState'}>
                  Spring <SiSpring className='station spring'/>
                </label>
              </div>
            </div>
            <div className="aling-form">
              <label>
                Countries: {values.countries.length !== 0?(<></>):(<label className="rejected tipe-text1">you must select at least one country</label>)}
              </label>
            </div>
          ...
          <div id="register-img2" className="aling-form">
            <label>
              Url image: <label className="accepted tipe-text1">opcional</label>
            </label>
            <input id="register-img" className="register-img" onChange={handleChange} value={values.imgUrl} name="imgUrl" type="text"/>
          </div>
